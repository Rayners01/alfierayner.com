...
export default function VisitedCountriesTile({ onStartFlying, onStopFlying }: VisitedCountriesTileProps) {
  ...
  const [velocity, setVelocity] = useState<{ vx: number; vy: number }>({ vx: 0, vy: 0 });
  const [planePos, setPlanePos] = useState<{ x: number; y: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      if (bgRef.current) {
        angleRad += speed;
        currentPos.current.x = 0.5 + radius * Math.cos(angleRad);
        currentPos.current.y = 0.5 + radius * Math.sin(angleRad);

        const bgPosX = `${currentPos.current.x * 100}%`;
        const bgPosY = `${currentPos.current.y * 100}%`;
        ...
        if (flying && planePos) {
          setPlanePos(prev => prev ? {
            ...
            y: prev.y + velocity.vy * window.innerHeight
          } : null);
        }
  ...
  const planeElement = (
    <div
      ref={planeRef}
      style={{
        position: flying ? 'absolute' : 'absolute',
        top: flying && planePos ? `${planePos.y}px` : '50%',
        left: flying && planePos ? `${planePos.x}px` : '50%',
  ...
  return (
    <div
      className="relative rounded-md w-full h-full bg-black cursor-pointer"
      onClick={handleClick}
    >
      <div
        ref={bgRef}
        className="absolute inset-0 bg-[url('/assets/world-map.jpg')] bg-[length:400%] bg-no-repeat opacity-80"
        style={{ backgroundPosition: '0% 0%' }}
      />
      {flying && planePos ? createPortal(planeElement, document.body) : planeElement}
    </div>
  );
}